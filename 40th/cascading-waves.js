...
function draw(e) {
  ...
  let time = e * 0.00001; //0.000008;
  let timeStep = 0.01;
  let grad = ctx.createLinearGradient(-width, 0, width, height);
  let t = time % 1;
  let tSide = floor(time % 2) === 0;
  let hueA = tSide ? 220 : 210;
  let hueB = !tSide ? 220 : 210;
  let colorA = hsl(hueA, 240, 15);
  let colorB = hsl(hueB, 240, 15);
  ...
  for (let j = 0; j < yCount; j++) {
    ...
    for (let i = 0; i < xCount; i++) {
      ...
      let y = n * width_half; //n * height_half;
      let x = t * (height + 20) - height_half - 10; //t * (width + 20) - width_half - 10;
      (i ? lineTo : moveTo)(y, x);
    }
    time += timeStep;
  }
  ...
  ctx.filter = 'blur(2px)';
  stroke(grad, 4); //stroke(grad, 5);
  //ctx.filter = 'blur(5px)';
  stroke(hsl(200, 100, 30, 0.7), 8);   // 4
}
